...
export const StudyRoom: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-100px)] max-w-6xl mx-auto p-6">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-4">
                    <Button
                        variant={showLab ? 'primary' : 'outline'}
                        size="sm"
                        onClick={() => setShowLab(!showLab)}
                    >
                        {showLab ? 'Close Lab' : '🚀 Open Physics Lab'}
                    </Button>
            ...
            <div className={`flex-1 flex gap-6 overflow-hidden ${showLab ? 'md:flex-row flex-col' : 'flex-col'}`}>
                {showLab && roomId && (
                    <InteractiveLab roomId={roomId} socket={socket} />
                )}

                <div className={`flex flex-col bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm ${showLab ? 'md:w-80 w-full' : 'flex-1'}`}>
